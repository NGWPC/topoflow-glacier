...
from topoflow_glacier import (
    BMI_base,
    model_input,  # model_output, rtg_files, time_utils
)
from topoflow_glacier import solar_funcs as solar
...
class met_component(BMI_base.BMI_component):
    ...
    def initialize_computed_vars(self):
        ...
        self.P_snow_3day_grid = np.zeros(
            (P_snow_3day.size, self.P_snow_3day_grid.shape[0], self.P_snow_3day_grid.shape[1])
        )
        # ------------------------------------------
    ...
    def update_saturation_vapor_pressure(self, MBAR=False, SURFACE=False):
        ...
        if self.DEBUG:
            if SURFACE:
                ...
                print("  e_sat_surf_min, e_sat_surf_max =", e_sat_surf_min, ", ", e_sat_surf_max)
                print()
            else:
                e_sat_air_min = self.e_sat_air.min()
                e_sat_air_max = self.e_sat_air.max()
                print("  e_sat_air_min, e_sat_air_max =", e_sat_air_min, ", ", e_sat_air_max)
                print()
    ...
    def update_vapor_pressure(self, SURFACE=False):
        ...
        if self.DEBUG:
            if SURFACE:
                ...
                print("  e_surf_min, e_surf_max =", e_surf_min, ", ", e_surf_max)
                print()
            else:
                e_air_min = self.e_air.min()
                e_air_max = self.e_air.max()
                print("  e_air_min, e_air_max =", e_air_min, ", ", e_air_max)
                print()
    ...
    def open_input_files(self):
        ...
        self.P_unit = model_input.open_file(self.P_type, self.P_file, NGEN_CSV=self.NGEN_CSV)
        ...
        # -----------------------------------------------
        self.albedo_unit = model_input.open_file(self.albedo_type, self.albedo_file)
        self.em_surf_unit = model_input.open_file(self.em_surf_type, self.em_surf_file)
        self.dust_atten_unit = model_input.open_file(self.dust_atten_type, self.dust_atten_file)
        self.cloud_factor_unit = model_input.open_file(self.cloud_factor_type, self.cloud_factor_file)
        self.canopy_factor_unit = model_input.open_file(self.canopy_factor_type, self.canopy_factor_file)
        # ----------------------------------------------------------------------------
    ...
    def read_input_files(self):
        ...
        P = model_input.read_next(
            self.P_unit, self.P_type, rti, units_factor=P_units_factor, NGEN_CSV=self.NGEN_CSV
        )
    ...
    def close_input_files(self):
        ...
        if self.T_surf_type != "Scalar":
            self.T_surf_unit.close()
        if self.RH_type != "Scalar":
            self.RH_unit.close()
        if self.p0_type != "Scalar":
            self.p0_unit.close()
        if self.uz_type != "Scalar":
            self.uz_unit.close()
        if self.z_type != "Scalar":
            self.z_unit.close()
        if self.z0_air_type != "Scalar":
            self.z0_air_unit.close()

        # ---------------------------------------------------
        # These are needed to compute Qn_SW and Qn_LW.
        # ---------------------------------------------------
        if self.albedo_type != "Scalar":
            self.albedo_unit.close()
        if self.em_surf_type != "Scalar":
            self.em_surf_unit.close()
        if self.dust_atten_type != "Scalar":
            self.dust_atten_unit.close()
        if self.cloud_factor_type != "Scalar":
            self.cloud_factor_unit.close()
        if self.canopy_factor_type != "Scalar":
            self.canopy_factor_unit.close()
    ...
    def open_output_files(self):
        ...
        if self.SAVE_ALB_GRIDS:
            model_output.open_new_gs_file(
                self, self.alb_gs_file, self.rti, var_name="alb", long_name="albedo", units_name="none"
            )
        # --------------------------------------
        ...
        if self.SAVE_EMA_PIXELS:
            model_output.open_new_ts_file(
                self, self.ema_ts_file, IDs, var_name="ema", long_name="air_emissivity", units_name="none"
            )
        ...
        if self.SAVE_ALB_PIXELS:
            model_output.open_new_ts_file(
                self, self.alb_ts_file, IDs, var_name="alb", long_name="albedo", units_name="none"
            )
    ...
    def close_output_files(self):
        if self.SAVE_EA_GRIDS:
            model_output.close_gs_file(self, "ea")
        if self.SAVE_ES_GRIDS:
            model_output.close_gs_file(self, "es")
        if self.SAVE_QSW_GRIDS:
            model_output.close_gs_file(self, "Qsw")
        if self.SAVE_QLW_GRIDS:
            model_output.close_gs_file(self, "Qlw")
        if self.SAVE_EMA_GRIDS:
            model_output.close_gs_file(self, "ema")
        if self.SAVE_TSURF_GRIDS:
            model_output.close_gs_file(self, "tsurf")
        if self.SAVE_ALB_GRIDS:
            model_output.close_gs_file(self, "alb")
        # -------------------------------------------------------------------
        if self.SAVE_EA_PIXELS:
            model_output.close_ts_file(self, "ea")
        if self.SAVE_ES_PIXELS:
            model_output.close_ts_file(self, "es")
        if self.SAVE_QSW_PIXELS:
            model_output.close_ts_file(self, "Qsw")
        if self.SAVE_QLW_PIXELS:
            model_output.close_ts_file(self, "Qlw")
        if self.SAVE_EMA_PIXELS:
            model_output.close_ts_file(self, "ema")
        if self.SAVE_TSURF_PIXELS:
            model_output.close_ts_file(self, "tsurf")
        if self.SAVE_ALB_PIXELS:
            model_output.close_ts_file(self, "alb")
