...
#   read_next2()
# -------------------------------------------------------------------
def read_next(file_unit, var_type, rti, dtype="float32", units_factor=1.0, NGEN_CSV=False):
    if var_type.lower() == "scalar":
        # -------------------------------------------
        # Scalar value was entered by user already
        # -------------------------------------------
        ...
        if NGEN_CSV:
            ...
            col = 0
            delim = " "
        data = read_scalar(file_unit, dtype, col=col, delim=delim)
    elif var_type.lower() in ["grid", "grid_sequence"]:
        # --------------------------------------
        # Single grid: Read grid from file
        # read_grid() accounts for byte order
...
#   read_next()
# -------------------------------------------------------------------
def read_scalar(file_unit, dtype="float32", col=0, delim=" "):
    # -------------------------------------------------
