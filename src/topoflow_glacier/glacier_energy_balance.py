...
class glacier_component(glacier_base.glacier_component):
    ...
    _output_var_names = [
        ...
        "cryosphere__domain_time_integral_of_melt_volume_flux",
    ]  # vol_M_total
    ...
    def close_input_files(self):
        if self.Cp_snow_type != "Scalar":
            self.Cp_snow_unit.close()
        if self.rho_snow_type != "Scalar":
            self.rho_snow_unit.close()
        if self.T0_type != "Scalar":
            self.T0_unit.close()
        if self.h0_snow_type != "Scalar":
            self.h0_snow_unit.close()
        if self.h0_swe_type != "Scalar":
            self.h0_swe_unit.close()
        if self.Cp_ice_type != "Scalar":
            self.Cp_ice_unit.close()
        if self.rho_ice_type != "Scalar":
            self.rho_ice_unit.close()
        if self.h0_ice_type != "Scalar":
            self.h0_ice_unit.close()
        if self.h0_iwe_type != "Scalar":
            self.h0_iwe_unit.close()
        if self.h_active_layer_type != "Scalar":
            self.h_active_layer_unit.close()
