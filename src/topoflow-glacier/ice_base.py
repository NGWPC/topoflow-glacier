...
from topoflow.utils import BMI_base, model_output, rtg_files
...
class ice_component(BMI_base.BMI_component):
    ...
    def initialize_vars_rtg(self):
        ...
        self.z_bed = rtg_files.read_grid(DEM_file, self.rti, RTG_type=self.rti.data_type)
        ...
        if self.H0_file.upper() == "NONE":
            ...
            self.h_ice = rtg_files.read_grid(H0_file, self.rti, RTG_type="FLOAT")
    ...
    def update_rectangle(self):
        ...
        # ---------------------------------------------------------------------
        if (self.COMPRESS_TOGGLE) and (self.h_ice.max() > 1) and (self.RESTART_TOGGLE != 2):
    ...
    def open_output_files(self):
        ...
        if self.SAVE_ZI_GRIDS:
            model_output.open_new_gs_file(
                self, self.zi_gs_file, self.rti, var_name="zi", long_name="ice_free_surface", units_name="m"
            )

        if self.SAVE_MR_GRIDS:
            model_output.open_new_gs_file(
                self, self.imr_gs_file, self.rti, var_name="imr", long_name="ice_meltrate", units_name="mm/hr"
            )
            #####  units_name='m/s')
    ...
    def close_output_files(self):
        ...
        if self.SAVE_ZI_GRIDS:
            model_output.close_gs_file(self, "zi")
        if self.SAVE_MR_GRIDS:
        ...
        if self.SAVE_ZI_PIXELS:
            model_output.close_ts_file(self, "zi")
        if self.SAVE_MR_PIXELS:
    ...
    def save_pixel_values(self):
        ...
        if self.SAVE_ZI_PIXELS:
            model_output.add_values_at_IDs(self, time, self.z_ice, "zi", IDs)

        if self.SAVE_MR_PIXELS:
            MR_mmph = self.mr_ice * self.mps_to_mmph
            model_output.add_values_at_IDs(self, time, MR_mmph, "imr", IDs)
