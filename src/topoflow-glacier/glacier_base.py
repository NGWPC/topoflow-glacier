...
class glacier_component(BMI_base.BMI_component):
    ...
    def open_output_files(self):
        ...
        if self.SAVE_IMR_GRIDS:
            model_output.open_new_gs_file(
                self, self.imr_gs_file, self.rti, var_name="imr", long_name="ice_meltrate", units_name="m/s"
            )
        if self.SAVE_HI_GRIDS:
            model_output.open_new_gs_file(
                self, self.hi_gs_file, self.rti, var_name="hi", long_name="ice_depth", units_name="m"
            )
        ...
        if self.SAVE_IMR_PIXELS:
            model_output.open_new_ts_file(
                self, self.imr_ts_file, IDs, var_name="imr", long_name="ice_meltrate", units_name="m/s"
            )

        if self.SAVE_HI_PIXELS:
            model_output.open_new_ts_file(
                self, self.hi_ts_file, IDs, var_name="hi", long_name="ice_depth", units_name="m"
            )

        if self.SAVE_IW_PIXELS:
            model_output.open_new_ts_file(
                self, self.iw_ts_file, IDs, var_name="iw", long_name="ice_water_equivalent", units_name="m"
            )
        if self.SAVE_CCI_PIXELS:
            model_output.open_new_ts_file(
                self, self.cci_ts_file, IDs, var_name="cci", long_name="ice_cold_content", units_name="J/m^2"
            )
    ...
    def close_output_files(self):
        if self.SAVE_SMR_GRIDS:
            model_output.close_gs_file(self, "smr")
        if self.SAVE_HS_GRIDS:
            model_output.close_gs_file(self, "hs")
        if self.SAVE_SW_GRIDS:
            model_output.close_gs_file(self, "sw")
        if self.SAVE_CC_GRIDS:
            model_output.close_gs_file(self, "cc")
        if self.SAVE_IMR_GRIDS:
            model_output.close_gs_file(self, "imr")
        if self.SAVE_HI_GRIDS:
            model_output.close_gs_file(self, "hi")
        if self.SAVE_CCI_GRIDS:
            model_output.close_gs_file(self, "cci")
        if self.SAVE_TMR_GRIDS:
            model_output.close_gs_file(self, "tmr")
        # -----------------------------------------------------------------
        if self.SAVE_SMR_PIXELS:
            model_output.close_ts_file(self, "smr")
        if self.SAVE_HS_PIXELS:
            model_output.close_ts_file(self, "hs")
        if self.SAVE_SW_PIXELS:
            model_output.close_ts_file(self, "sw")
        if self.SAVE_CC_PIXELS:
            model_output.close_ts_file(self, "cc")
        if self.SAVE_IMR_PIXELS:
            model_output.close_ts_file(self, "imr")
        if self.SAVE_HI_PIXELS:
            model_output.close_ts_file(self, "hi")
        if self.SAVE_CCI_PIXELS:
            model_output.close_ts_file(self, "cci")
        if self.SAVE_TMR_PIXELS:
            model_output.close_ts_file(self, "tmr")
